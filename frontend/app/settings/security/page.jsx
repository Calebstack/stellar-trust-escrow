    ...
    <div className={styles.container}>
      ...
      {!setupComplete && (
        <div>
          ...
          {qrDataUrl ? (
            <img src={qrDataUrl} alt="QR code for 2FA" aria-label="QR code for 2FA, manual entry alternative available" width={QR_SIZE} height={QR_SIZE} />
          ) : (
            <p>Loading QR code…</p>
          )}
          ...
          <div id="totp-desc">Enter the 6‑digit code from your authenticator app. Auto‑submits when 6 digits are entered.</div>
          {verifying && <p>Verifying…</p>}
        </div>
      )}
      {setupComplete && (
        <div>
          <button onClick={() => setShowModal(true)} disabled={disabling}>Disable 2FA</button>
          {showModal && (
            <div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="modal-title">
              ...
              <pre className={styles.backupGrid}>{backupCodes.map((c, i) => (<span key={i}>{c}</span>)).join('\n')}</pre>
              <button onClick={copyAll}>Copy all</button>
              <button onClick={downloadTxt}>Download as .txt</button>
              <div>
                <label>
                  <input type="checkbox" checked={savedChecked} onChange={e => setSavedChecked(e.target.checked)} /> I've saved these
                </label>
              </div>
              <button onClick={() => setShowModal(false)} disabled={!savedChecked}>Close</button>
              <hr />
              <h3>Disable 2FA</h3>
              <label>
                Current TOTP:
              ...
              <button onClick={handleDisable} disabled={disableTotp.length !== 6 || disabling}>Confirm Disable</button>
            </div>
          )}
        </div>
      )}
